<?php
/**
 * Validates the presence of essential environment variables necessary for the
 * application to run correctly. Specifically checks for variables related to
 * directories and database configuration. It also ensures that the test and
 * preparation directories are the same when running locally without SSH
 * connection requirements.
 * This function will issue error messages through `error_message()` for any
 * missing environment variables and logs a message upon successful validation
 * of all required variables.
 *
 * @param bool $check_db Optional. Whether to include database-related
 * environment variables in the check. Defaults to true. If set to false,
 * database variables (prefixed with 'WPT_DB_') are not checked.
 *
 * @return void This function does not return a value but will halt execution if
...
function check_required_env( $check_db = true ) {
	...
	if ( empty( getenv( 'WPT_SSH_CONNECT' ) ) && getenv( 'WPT_TEST_DIR' ) !== getenv( 'WPT_PREPARE_DIR' ) ) {
...
function error_message( $message ) {
	fwrite( STDERR, 'Error: ' . $message . PHP_EOL ); // Write error message to standard error output.
}
...
function get_env_details() {
	...
	$env = array(
		...
		'mysql_version' => '', // Will be set later if MySQL is available.
		'system_utils'  => array(),
	...
	$WPT_DB_HOST     = trim( getenv( 'WPT_DB_HOST' ) ?: 'localhost' ); // Default to localhost if not set.
	...
	if ( class_exists( 'mysqli' ) ) {
		...
		if ( ! $mysqli->connect_error ) {
			$env['mysql_version'] = $mysqli->query("SELECT VERSION()")->fetch_row()[0]; // Get MySQL version.
			$mysqli->close();
	  }
	}
